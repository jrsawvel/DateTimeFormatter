...
my %hash = DateTimeFormatter::create_date_time_stamp_local($time);
print Dumper(\%hash);
...
%hash = DateTimeFormatter::create_date_time_stamp_utc($e);
print Dumper(\%hash);

print "displaying output from my variables submitted in string to sub\n";
print DateTimeFormatter::create_date_time_stamp_local($format);
